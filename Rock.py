...
ERROR_SUGGESTIONS = {
    ...
    "invalid_global_initializer": "Global variables must be initialized with a constant expression (e.g., number or string literal)."
}
...
def parse(tokens: List[Token]) -> Program:
    ...
    def parse_global(is_function_scope: bool, func_name: str = None, body: List[ASTNode] = None, variables: Dict[str, str] = None) -> None:
        ...
        if var_type == "int" and isinstance(expr, NumberExpr) and expr.type != "i64":
            ...
            valid = False
        elif isinstance(expr, VarExpr) or isinstance(expr, BinaryExpr):
            log_error(f"Global variable '{var_name}' must be initialized with a constant expression", var_token.line, ERROR_SUGGESTIONS["invalid_global_initializer"])
            valid = False
    ...
    return Program(globals=globals, functions=functions, invoke=invoke)
    ...
    def process_expr(expr: Expression, func_name: str, builder: ir.IRBuilder) -> Tuple[ir.Value, ir.Type]:
        if isinstance(expr, NumberExpr):
            ...
            if isinstance(left_type, ir.IntType):
                ...
                return op_map[expr.op](left_val, right_val), left_type
    ...
    try:
        ...
        for var_name, (var_type, expr) in program.globals.items():
            ...
            if isinstance(expr, NumberExpr):
                ...
                log_error(f"Global variable '{var_name}' must be initialized with a constant", None, ERROR_SUGGESTIONS["invalid_global_initializer"])
                sys.exit(1)
        ...
        for func_name, func in program.functions.items():
            ...
            for stmt in func.body:
                if isinstance(stmt, GlobalVarDecl):
                    continue  # Handled at module level
        ...
        if program.invoke:
            called_func = module.get_global(program.invoke)
            builder.call(called_func, [])
